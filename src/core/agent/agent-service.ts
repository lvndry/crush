  ...
  validateAgentConfig(config: AgentConfig): Effect.Effect<void, AgentConfigurationError> {
    return Effect.gen(function* (this: DefaultAgentService) {
      ...
      if (config.timeout && (config.timeout < 1000 || config.timeout > 3600000)) {
        return yield* Effect.fail(new AgentConfigurationError({
          agentId: "unknown",
          field: "timeout",
...
export function createAgent(name: string, description: string, config?: Partial<AgentConfig>): Effect.Effect<Agent, StorageError | AgentAlreadyExistsError | AgentConfigurationError | ValidationError, AgentService> {
  return Effect.gen(function* () {
